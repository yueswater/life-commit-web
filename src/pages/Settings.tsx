  ...
  return (
    <div className="w-full max-w-6xl flex flex-col lg:flex-row gap-12 py-10 px-6">
      <div className="w-full lg:w-1/3 flex flex-col items-center">
        ...
        <h2 className="mt-6 text-3xl font-black italic tracking-tighter text-primary">
          @{profile?.username}
        </h2>
      </div>
      ...
      <div className="w-full lg:w-2/3">
        <div className="tabs tabs-bordered mb-8">
          <button
            className={`tab tab-lg font-bold pb-4 ${activeTab === 'account' ? 'tab-active text-primary border-primary' : 'text-gray-500'}`}
            onClick={() => setActiveTab('account')}
          >
            帳戶資訊
          </button>
          <button
            className={`tab tab-lg font-bold pb-4 ${activeTab === 'security' ? 'tab-active text-primary border-primary' : 'text-gray-500'}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-300">
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-bold text-gray-500 uppercase tracking-widest text-xs">
                  使用者名稱
                </span>
              </label>
              <div className="text-xl font-bold py-2 border-b border-base-300">
                {profile?.username}
              </div>
            </div>
            <div className="form-control w-full">
          ...
          <div className="flex flex-col gap-10 animate-in fade-in duration-300">
            <form onSubmit={handleUpdateEmail} className="flex flex-col gap-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  className="btn btn-primary h-14 rounded-2xl px-8 font-bold"
                  type="submit"
                  disabled={loading}
                >
                  儲存
            ...
            <div className="flex flex-col gap-4">
              ...
              <button
                ...
                className="btn btn-outline border-gray-700 h-14 rounded-2xl flex items-center gap-3 hover:bg-error hover:text-white hover:border-error group transition-all"
                disabled={loading}
              >
