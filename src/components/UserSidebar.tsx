...
import { X, Settings, LogOut, User as UserIcon, LayoutDashboard } from 'lucide-react';
    ...
    <>
      <div 
        className={`fixed inset-0 z-[150] transition-all duration-500 ${
          isOpen ? 'bg-black/40 backdrop-blur-md opacity-100' : 'bg-transparent opacity-0 pointer-events-none'
        }`}
        onClick={onClose}
      />

      <div className={`fixed top-0 right-0 h-full w-full max-w-[320px] bg-base-100 z-[160] shadow-2xl transition-transform duration-500 ease-out border-l border-base-300/30 ${
        isOpen ? 'translate-x-0' : 'translate-x-full'
      }`}>
        <div className="flex flex-col h-full p-8">
          <div className="flex justify-between items-center mb-10">
            <button onClick={onClose} className="btn btn-ghost btn-circle hover:bg-base-200">
              <X size={28} />
            </button>
          ...
          <div className="flex flex-col items-center mb-10">
            ...
            <p className="text-[10px] font-bold text-primary uppercase tracking-widest opacity-80">{t('stats.plan')}</p>
          </div>
...
const SidebarItem = ({ icon, label, to, onClick }: { icon: React.ReactNode, label: string, to: string, onClick: () => void }) => (
  <Link 
    to={to} 
    onClick={onClick}
