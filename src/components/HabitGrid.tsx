...
import { format, subDays, isSameDay, startOfToday, eachMonthOfInterval, isSameMonth } from 'date-fns';
...
const HabitGrid = ({ habitId }: HabitGridProps) => {
  ...
  const days = useMemo(() => {
    return Array.from({ length: totalDays }, (_, i) => subDays(today, totalDays - 1 - i));
  }, [today]);
  ...
  const months = useMemo(() => {
    ...
    return interval.map((monthDate) => {
      const firstDayOfMonthIndex = days.findIndex((d) => isSameMonth(d, monthDate));
  ...
  const getLevel = (date: Date) => {
    const log = commits.find((l) => isSameDay(new Date(l.execution_date), date));
    if (!log) return 0;
  ...
  return (
    <div className="w-full flex flex-col items-center p-8 bg-[#161b22]/40 rounded-[2.5rem] border border-gray-800 shadow-2xl overflow-x-auto">
      ...
      <div className="w-full max-w-[800px] mt-6 flex justify-end items-center gap-3 px-4 shrink-0">
        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{t('stats.less')}</span>
        ...
        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{t('stats.more')}</span>
      </div>
    </div>
  );
};
