...
const TagSearch = ({ selectedTagId, onSelect }: TagSearchProps) => {
  ...
  const fetchTags = async (searchQuery: string) => {
    setLoading(true);
    let rpc = supabase.from('tags').select('id, name').order('created_at', { ascending: false });
  ...
  return (
    <div className="form-control relative w-full" ref={dropdownRef}>
      <label className="label py-1 px-0">
        <span className="label-text text-gray-400 font-bold uppercase text-xs">{t('habit.tag')}</span>
      </label>
      <div className="flex items-center gap-3 bg-base-200 border border-gray-700 h-14 rounded-2xl px-4 focus-within:border-primary transition-all w-full">
        <Tag size={18} className={selectedTagId ? "text-primary" : "text-gray-500"} />
        <input
          type="text"
        ...
        />
        {loading ? <Loader2 size={16} className="animate-spin text-gray-500" /> : <Search size={16} className="text-gray-500" />}
      </div>
        ...
        <div className="absolute top-[88px] left-0 w-full bg-base-300 border border-gray-700 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="p-2 flex flex-col gap-1 max-h-60 overflow-y-auto">
            {results.length > 0 ? (
              ...
              <div className="p-4 text-center text-gray-500 text-sm">{t('habit.noMatchingTags')}</div>
            )}
